...
function DeleteBoothTypeConfirm({
  ...
  return (
    <div className="flex flex-col gap-2">
      <p className="text-sm">
        {boothCount > 0
          ? `Delete ${boothType.name}? This also deletes ${boothCount} booth${boothCount === 1 ? "" : "s"} using it. This can't be undone.`
          : `Delete ${boothType.name}? This can't be undone.`}
      </p>
...
export function BoothTypeList({
  ...
  boothCountByType: Record<string, number>;
  showId: string;
}) {
        ...
        >
          {mode?.id === boothType.id && mode.type === "edit" ? (
            ...
            <DeleteBoothTypeConfirm
              boothType={boothType}
              showId={showId}
              boothCount={boothCountByType[boothType.id] ?? 0}
