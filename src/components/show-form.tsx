...
import { createShow, type ShowFormState } from "@/lib/actions/shows";

const initialState: ShowFormState = { error: null };

export function ShowForm({ organiserId }: { organiserId: string }) {
  const createShowForOrganiser = createShow.bind(null, organiserId);
  const [state, formAction, pending] = useActionState(createShowForOrganiser, initialState);
    ...
    <form action={formAction} className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          rows={3}
          placeholder="e.g. Zelle to payments@example.com, or cash on arrival"
          className="rounded-lg border border-zinc-300 px-4 py-3 text-base dark:border-zinc-700 dark:bg-zinc-900"
        />
      </div>
